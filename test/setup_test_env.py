...
from test_config import TEST_DB_CONFIG, PROJECT_ROOT
    ...
    try:
        ...
        with engine.connect() as connection:
            ...
            if result.fetchone():
                ...
                # 确保删除用户前删除其拥有的数据库
                if database_exists(
                    f"postgresql://{TEST_DB_CONFIG['admin_user']}:{TEST_DB_CONFIG['admin_password']}@{TEST_DB_CONFIG['host']}:{TEST_DB_CONFIG['port']}/{TEST_DB_CONFIG['database']}"
                ):
                    drop_database(
                        f"postgresql://{TEST_DB_CONFIG['admin_user']}:{TEST_DB_CONFIG['admin_password']}@{TEST_DB_CONFIG['host']}:{TEST_DB_CONFIG['port']}/{TEST_DB_CONFIG['database']}"
                    )
...
DB_CONNECTION_STRING = "postgresql://{TEST_DB_CONFIG['user']}:{TEST_DB_CONFIG['password']}@{TEST_DB_CONFIG['host']}:{TEST_DB_CONFIG['port']}/{TEST_DB_CONFIG['database']}"
